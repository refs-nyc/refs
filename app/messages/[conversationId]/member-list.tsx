...
export default function MemberListScreen() {
  ...
  return ( 
    <View style={{ flex: 1, backgroundColor: c.surface }}>
      <YStack gap={s.$1} style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'start', paddingTop: s.$5 }}>
        <XStack gap={s.$1} style={{ alignItems: 'center', padding: s.$3, paddingTop: s.$7, width: '100%'}}>
          <Pressable onPress={() => { router.dismissTo(`/messages/${conversationId}`) }}>
        ...
        <ScrollView>
          <YStack gap={s.$1} style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'start', paddingHorizontal: s.$4 }}>
            {members.map(m =>
              <View key={m.id}>
