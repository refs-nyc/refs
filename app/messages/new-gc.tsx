...
export default function NewGCScreen() 
{
  ...
  useEffect(() => {
    const getUsers = async () => {
      const ids = typeof queryParams.members === 'string' ? queryParams.members.split(',') : queryParams.members;
  ...
  const getUserListString = (users: Profile[]) => {
    ...
    if (users.length > 1) s += ' and ' + (users.length > 3 ? `${users.length-3} others` : users[users.length-1].firstName);
