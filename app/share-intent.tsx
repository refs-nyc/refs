...
const WebUrlComponent = ({ shareIntent }: { shareIntent: ShareIntentType }) => {
  return (
    <View style={[styles.gap, styles.row, { borderWidth: 1, borderRadius: 5, height: 102 }]}>
      <Image
        source={
          shareIntent.meta?.['og:image'] ? { uri: shareIntent.meta?.['og:image'] } : undefined
        }
        style={[styles.icon, styles.gap, { borderRadius: 5 }]}
      />
      <View style={{ flexShrink: 1, padding: 5 }}>
        <Text style={[styles.gap]}>{shareIntent.meta?.title || '<NO TITLE>'}</Text>
...
export default function ShareIntent() {
  ...
  return (
    <View style={styles.container}>
      ...
      {shareIntent?.type === 'weburl' && <WebUrlComponent shareIntent={shareIntent} />}
      {shareIntent?.files?.map((file) => (
        <Image key={file.path} source={{ uri: file.path }} style={[styles.image, styles.gap]} />
      ))}
