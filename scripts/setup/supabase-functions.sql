...
CREATE OR REPLACE FUNCTION process_new_item(item_id TEXT, ref_id TEXT, creator TEXT, item_text TEXT, ref_title TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  INSERT INTO items (id, ref, creator, text, seven_string, seven_string_embedding)
  VALUES (item_id, ref_id, creator, item_text, seven_string, embedding_vector)
...
BEGIN
  ...
  SELECT array_agg(i.seven_string ORDER BY i.created DESC) INTO seven_strings
  FROM items i
