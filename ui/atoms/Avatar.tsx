...
export const AvatarStack = ({ sources, size = s.$3 }: { sources: (string | undefined)[]; size: number }) => 
  {
      ...
      <YStack gap={0}>
        ...
        >
            {shownSources.map((source, index) => (
              <View key={index} style={{position: 'relative', left: (-size*index) + index*(xSpacing), top: index*ySpacing }}>
                <Avatar source={source} size={size} />
              </View>
            ))}
        </XStack>
        {others > 0 &&
          <Text> and <Text style={{fontWeight: 'bold'}}>{others} others</Text></Text>
        }
      </YStack>
    </>
  )
